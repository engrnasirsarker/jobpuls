<script setup>
    ...
    const fetchData = async()=>{
        try {
            const res = await axios.get('/api/student-group')
    ...
    const columns = [
        ...
        {label:"Name",field:"subject_name",type:"string"},
        {label:"Action",field:"action",width:'20%'},
    ]
    const createItem = async () => { 
        try {
            await axios.post(`/api/student-group`, form.value)       
            fetchData();
            form.value = { };  
    ...
    const editItem = (item) => { 
        editForm.value.subject_name = item.subject_name;
    ...
    const updateItem = async () => {
        ...
        try {
            await axios.patch(`/api/student-group/${id}`, editForm.value);
            Toast.fire({ icon: 'success', title: 'Update success !' });
            fetchData();
            isShowEditModal.value = false
            router.push('/admin/student-group');
        } catch (error) {        
            errors.value = error.response.data.errors;
        }
    };
    const deleteItem = (id)=>{
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            if (result.value) {
                axios.delete('/api/student-group/'+id)
                .then(fetchData())
...
<template>    
    <BreadcrumbComponent :currentPage="'Student Group'"/> 
