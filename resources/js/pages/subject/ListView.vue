<script setup>
    ...
    const fetchData = async()=>{
        try {
            const res = await axios.get('/api/student-subject')
    ...
    const createItem = async () => { 
        try {
            await axios.post(`/api/student-subject`, form.value)       
            fetchData();
            form.value = { };  
    ...
    const updateItem = async () => {
        ...
        try {
            await axios.patch(`/api/student-subject/${id}`, editForm.value);
            Toast.fire({ icon: 'success', title: 'Update success !' });
            fetchData();
            isShowEditModal.value = false
            router.push('/admin/student-subject');
        } catch (error) {        
            errors.value = error.response.data.errors;
        }
    };
    const deleteItem = (id)=>{
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            if (result.value) {
                axios.delete('/api/student-subject/'+id)
                .then(fetchData())
...
<template>    
    ...
    <div class="col-span-full xl:col-span-8 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 mx-3 mb-4">
        ...
        <div class="flex justify-between">
            <div>
              <h2 class="text-lg">All Subject</h2>
            </div>
            <div>
