<script setup>
    ...
    const columns = [
        ...
        {label:"Name",field:"shift_name",type:"string"},
        {label:"Action",field:"action",width:'20%'},
    ]
    const createItem = async () => { 
        try {
            await axios.post(`/api/shift`, form.value)       
            fetchData();
            form.value = { };  
    ...
    const editItem = (item) => { 
        editForm.value.shift_name = item.shift_name;
    ...
    const updateItem = async () => {
        ...
        try {
            await axios.patch(`/api/shift/${id}`, editForm.value);
...
<template>    
    <BreadcrumbComponent :currentPage="'Student Shift'"/> 
