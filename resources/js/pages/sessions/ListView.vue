<script setup>
    ...
    const columns = [
        ...
        {label:"Name",field:"session_name",type:"string"},
        {label:"Action",field:"action",width:'20%'},
    ]
    const createItem = async () => { 
        try {
            await axios.post(`/api/session`, form.value)       
            fetchData();
            form.value = { };  
    ...
    const editItem = (item) => { 
        editForm.value.session_name = item.session_name;
    ...
    const updateItem = async () => {
        ...
        try {
            await axios.patch(`/api/session/${id}`, editForm.value);
...
<template>    
    ...
    <div class="col-span-full xl:col-span-8 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 mx-3 mb-4">
        ...
        <div class="flex justify-between">
            <div>
              <h2 class="text-lg">All Session</h2>
            </div>
            <div>
